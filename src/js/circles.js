...
const imgContainersFor = (imgContainers, deg, firstObjectCoefficient) => {
  imgContainers.forEach((imgContainer, i) => {
    imgContainer.style.transform = `rotate(${(deg / (imgContainers.length - 1)) * (i - firstObjectCoefficient)}deg)`;
    imgContainer.querySelector('.circle__img').style.transform = `rotate(${((deg / (imgContainers.length - 1)) * (i - firstObjectCoefficient)) * -1}deg)`;
  });
};
